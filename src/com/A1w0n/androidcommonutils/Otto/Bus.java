 ...
 * 
 * <p>
 * The Bus allows publish-subscribe-style communication between components without requiring the
 ...
 * instance will determine the type of event and route it to all registered listeners.
 * 
 * <p>
 ...
 * 
 * <p>
 * Handlers should not, in general, throw. If they do, the Bus will wrap the exception and re-throw
 * it.
 * 
 * <p>
 * The Bus by default enforces that all interactions occur on the main thread. You can provide an
 * alternate enforcement by passing a {@link ThreadEnforcer} to the constructor.
 * 
 ...
 * called with the return value from the producer.
 * 
 * <h2>Dead Events</h2>
 * If an event is posted, but no registered handlers can accept it, it is considered "dead." To give
 * the system a second chance to handle dead events, they are wrapped in an instance of
 * {@link com.squareup.otto.DeadEvent} and reposted.
 * 
 * <p>
...
public class Bus {
	 ...
	 * 
	 * @param identifier
	 *            a brief name for this bus, for debugging purposes. Should be a valid Java
	 *            identifier.
	 */
	 ...
	 *            identifier.
	 * @param handlerFinder
	 *            Used to discover event handlers and producers when registering/unregistering an
	 *            object.
	 */
	 ...
	 * called immediately with the result of calling that producer.
	 * <p>
	 * If any producers are registering for types which already have subscribers, each subscriber
	 * will be called with the value from the result of calling the producer.
	 * 
	 * @param object
	 ...
	 * queued in-order of occurrence so they can be dispatched in the same order.
	 */
	...
	/**
	 * Throw a {@link RuntimeException} with given message and cause lifted from an
	 * {@link InvocationTargetException}. If the specified {@link InvocationTargetException} does
	 * not have a cause, neither will the {@link RuntimeException}.
	 */
